...
- In September 2026, 148 of 635 rows were single-recipe pages from `academy.claude.com/use-cases`.
...
**It moves only on positive evidence.** The first cut moved 119 rows in September 2026 because its default
...
77 rows changed level in September 2026: 67 down out of `builder`, 10 up into it.
...
wrong. That distinction is what kept those rows alive.
...
The collection cards are Sales, Legal, HR, Finance and Operations. A sixth was ruled
for the gallery's General tab and never built, because no roleless row turned out to be on
it. In September 2026, 40 individual rows were absorbed. Every absorbed row's
...
**And a pair of rows were removed outright.** Both are fundraising craft written for a
...
In September 2026 the catalogue went from 635 rows to 598. The share that is Anthropic's own writing went
...
a deliberate acceptance. That decision was made in September 2026, about a catalogue where those 148 rows were
...
- **The rest of the catalogue has never been swept.** Rule B was applied to the use-case
  gallery only. Run over everything in September 2026, the drop side reports 62 rows outside the gallery
...
- **In September 2026, 16 gallery rows had no family** in `data/use-case-departments.json` because the
