...
def main():
    # FIX-34. This used to print "missing - skipped" and carry on, and every one of these
    # five files is one the site cannot open without: items.js is the catalogue, picks.js
    # is the picks block, search-keywords.js is the search. A build that lost one of them
    # printed a line nobody reads, exited zero, and deployed a site with an empty Browse
    # page. Missing is not a state this can continue from.
    missing = [src for src, _dst, _var in PAIRS if not os.path.exists(src)]
    if missing:
        sys.exit("cannot mirror %s - every one of these is a file the site loads at "
                 "startup. Build it before this runs." % ", ".join(missing))
