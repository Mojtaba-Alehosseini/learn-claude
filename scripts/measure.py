...
def content_gaps():
    """The suite's content-gap rows, read straight from the file that records them."""
    path = os.path.join(ROOT, "scripts", "test-search.py")
...
def render(m, pc):
    m = dict(m)
    m.setdefault("content_gaps", content_gaps())
    L = []
    w = L.append
    ...
    w("### Questions the catalogue cannot answer (%d)" % len(m["content_gaps"]))
    w("")
    w("Queries from the suite that fail because nothing here is on the subject, not "
      "because the ranking is wrong. No stemmer, spelling pair or synonym can conjure a "
      "resource, so these are a harvest list rather than a search backlog. Generated from "
      "`scripts/test-search.py`; the judgement behind each one is in its reason there.")
    w("")
    if m["content_gaps"]:
        w("| the role that asked | what they typed | why it cannot be answered |")
        w("|---|---|---|")
        for role, q, why in m["content_gaps"]:
            w("| %s | %s | %s |" % (role, q, why))
    else:
        w("None recorded.")
    w("")
